...
public class MovementSystem : MonoBehaviour {
        ...
        for(int i = 0; i<CollisionData.contacts.Length; i++){
                ...
                if(CollisionData.contacts[i].point.y <  transform.position.y - ((capsule.height/2) - capsule.radius*0.95f)){
                    ...
                    if(advanced.maxSlopeAngle>0){
                        ...
                        else if(a<advanced.maxSlopeAngle+0.1f){advanced.isTouchingWalkable = true;}
    ...
    public class MS_Editor : Editor{
        ...
        public override void OnInspectorGUI(){
            ...
            GUILayout.Label("Movement System",new GUIStyle(GUI.skin.label){alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 16});
        ...
        #region Camera Setup
            ...
            EditorGUILayout.Space();
            t.cameraInputMethod = (MovementSystem.CameraInputMethod)EditorGUILayout.EnumPopup(new GUIContent("Input Method", "Determines the method used to rotate camera. \n\nTraditional uses the mouse on all axes. \nTraditional with constraints uses the mouse on the Y axis only. \nRetro uses Keybinds (left and right movement keys) to rotate the camera along the Y axis."),t.cameraInputMethod);
            if(t.cameraInputMethod == MovementSystem.CameraInputMethod.Traditional){t.verticalRotationRange = EditorGUILayout.Slider(new GUIContent("Vertical Rotation Range","Determines how much range does the camera have to move vertically."),t.verticalRotationRange,90,180);}
            if(t.cameraInputMethod == MovementSystem.CameraInputMethod.Traditional || t.cameraInputMethod == MovementSystem.CameraInputMethod.TraditionalWithConstraints){
            t.mouseInputInversion = (MovementSystem.InvertMouseInput)EditorGUILayout.EnumPopup(new GUIContent("Mouse Input Inversion","Determines if mouse input should be inverted, and along which axes"),t.mouseInputInversion);
        ...
        #region Audio/SFX Setup
            ...
            EditorGUILayout.Space();
            t.fsmode = (MovementSystem.FSMode)EditorGUILayout.EnumPopup(new GUIContent("Footstep Mode","Determines the method used to trigger footsetps."),t. fsmode);
            EditorGUILayout.Space();

            #region FS Static
            if(t.fsmode == MovementSystem.FSMode.Static){
                ...
                t.dynamicFootstep.materialMode = (MovementSystem.DynamicFootStep.matMode)EditorGUILayout.EnumPopup(new GUIContent("Material Type", "Determines the type of material will trigger footstep audio."),t.dynamicFootstep.materialMode);
                EditorGUILayout.Space();
                #region Wood Section
                ...
                if(showWoodFS){
                    GUILayout.BeginVertical("box");
                    if(t.dynamicFootstep.materialMode == MovementSystem.DynamicFootStep.matMode.physicMaterial){
